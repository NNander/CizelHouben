##' @export
es_regression_old_v3 <- function(data,
                              yvar,
                              xvars,
                              tte_var,
                              fe = 0,
  ...
  if (is.null(dir.output)){
    dir.output <- file.path(dir.output,'inst/output',Sys.Date())
  ...
  if (min(dt2plot[['x']]) >= 0){
    list(
      ...
      wald = NULL,
      est_plot = NULL,
      info = "Insufficient number of events"
    ) ->
      output
  } else {
    ...
    if ('try-error' %in% class(res[['waldtest']])){
      caption = 'Wald test failed.'
    } else {
      res[['waldtest']] %>>%
        subset(
          stat %in% c('CEGR[-8,-1]',
