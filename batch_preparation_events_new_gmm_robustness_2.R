##' @export
batch_preparation_events_new_gmm_robustness_2 <- function(pre_qrt = 9,
                                         post_qrt = 9){
  ...
  Projects2016.Macropru::prepare_MaP_events_update() %>>%
  ...
  (ref.imf %>>%
    ...
    mutate(
      type = classify_price.vs.quantity_robustness_2(measure)
    ) ->
    events.new2
